...
该算子的核心功能是：
1.  **标签解析**：识别并提取 `<chapter>`、`<qa_pair>`、`<question>`、`<answer>`、`<solution>` 和 `<label>` 等标签中的内容。
2.  **ID 还原**：将 LLM 返回的数字 ID 映射回原始的文本内容或图片标签（基于 `MinerU2LLMInputOperator` 生成的转换布局文件）。
3.  **资源同步**：自动将关联的图片从中间目录复制到最终输出目录，并修正图片引用路径。
...
* `<chapter>`: 章节区块，内含标题和多个 QA 对。
* `<title>`: 章节标题对应的 **ID**。
* `<qa_pair>`: 单个问答对区块。
* `<question>` / `<solution>`: 对应内容的 **ID** 列表（如 `1, 2, 5`）。
* `<answer>`: 从 solution 中提取出的答案，**是具体内容，不是ID**
* `<label>`: 题目类型或标签信息，**是真实序号，不是ID**。
...
假设 LLM 返回：`<question>1, 3</question>`
